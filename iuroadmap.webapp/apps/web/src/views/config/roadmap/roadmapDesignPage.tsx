...
// export function RoadmapDesignPage() {
...
// 							if (edgeToDelete?.data?.edgeId) {
// 								await deletePrerequisiteApi({ roadmapId: major.id, edgeId: edgeToDelete.data.edgeId });
// 							}
// 						}
...
// 							type: 'success',
// 							title: 'Course Updated',
// 							message: `Successfully updated "${form.name
// 					}". Added ${added.length} and removed ${removed.length} prerequisite(s).`,
// 			});
// 		closePanel();
// 		await loadGraph();
// 	} catch (err: any) {
...
// 			setSaving(true);
// 			try {
// 				await deleteCourseNodeApi({ roadmapId: major.id, courseNodeId: Number(selectedNodeId) });
// 				setNotification({
// 					type: 'success',
...
// 		const created = await createCourseNodeApi({ roadmapId: major.id, data: createPayload });

// 		for (const prerequisiteId of form.prerequisiteIds) {
// 			if (prerequisiteId === created.id) continue;

// 			await createPrerequisiteApi({ roadmapId: major.id, data: { course_node_id: created.id, prerequisite_node_id: prerequisiteId } });
// 		}
...
// 	setSaving(true);
// 	try {
// 		await updateCourseNodeApi({
// 			roadmapId: major.id, courseNodeId: courseNodeId, data: {
// 				slug: form.slug.trim(),
// 				name: form.name.trim(),
// 				credits,
// 				description: form.description.trim() || undefined,
...
// 		for (const prerequisiteId of added) {
// 			await createPrerequisiteApi({ roadmapId: major.id, data: { course_node_id: courseNodeId, prerequisite_node_id: prerequisiteId } });
// 		}
...
// 					title={
// 						<div>
// 							<div>{major ? `Major Roadmap: ${major.name}` : 'Major Roadmap Designer'}</div>
// 							<div style={{ fontSize: 14, color: '#666', fontWeight: 'normal', marginTop: 4 }}>
// 								Left-click a node to edit. Drag nodes to reposition and auto-save coordinates.
// 							</div>
// 						</div>
// 					}
// 				/>
