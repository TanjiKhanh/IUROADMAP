...
export function ExploreMajorsPage() {
    ...
    const { data: majorsData, isLoading: loading, error: queryError, refetch } = useExploreRoadmapsControllerGetMajors();
    const departments = majorsData?.data?.departments || [];
    const majors = majorsData?.data?.majors || [];
    const error = queryError ? (queryError as any).message || 'Failed to load majors.' : null;
    ...
    const majorsByDepartment = useMemo(() => {
        const map = new Map<string, { department: DepartmentFilterDto; majors: MajorCardDto[] }>();
        ...
        for (const major of filteredMajors) {
            const dept = major.department as any;
            const deptSlug = dept?.slug as string;
            if (!deptSlug || !dept) continue;
    ...
    const handleCloneMajor = async (major: MajorCardDto) => {
        try {
            setEnrollNotice(null);
    ...
    const handleViewMajor = async (major: MajorCardDto) => {
        try {
            setEnrollNotice(null);
    ...
    const getMajorBadge = (major: MajorCardDto) => {
    ...
    const renderDepartmentFilters = () => (
        <div className="explore-filter-row">
            ...
            {departments.map((dept) => (
                <button
                    key={dept.id}
                    className={`explore-chip ${selectedDept === dept.slug ? 'explore-chip-active' : ''
                        }`}
