...
let RolesGuard = class RolesGuard {
    ...
    canActivate(context) {
        ...
        if (!userRoles || userRoles.length === 0) {
            throw new common_1.ForbiddenException('User has no roles assigned');
        }
        const hasRequiredRole = requiredRoles.some((requiredRole) => userRoles.includes(requiredRole));
        if (!hasRequiredRole) {
            throw new common_1.ForbiddenException(`Insufficient permissions. Required roles: ${requiredRoles.join(', ')}. Your role: ${userRoles.join(', ')}`);
        }
        return true;
    }
};
